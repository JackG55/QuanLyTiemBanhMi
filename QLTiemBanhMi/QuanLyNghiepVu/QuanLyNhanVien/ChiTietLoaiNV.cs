...
{
    public partial class ChiTietLoaiNV : Form
    {
        public ChiTietLoaiNV(NhanVien nhanVien)
        {
            InitializeComponent();
        }
        ...
        {
            tb_id.Text = Program.chiTietLoaiNV.Id;
            tb_tenloainhanvien.Text = Program.chiTietLoaiNV.Tenloai ;
            tb_ghichu.Text = Program.chiTietLoaiNV.Ghichu;
        ...
        public bool KiemTra()
        {
            if (tb_id.Text == "" || tb_tenloainhanvien.Text == "")
            {
            ...
            if (Program.opt == 1)
            {
                if (list_tenloainv.Contains(tb_tenloainhanvien.Text))
                {
        ...
        private void buttonLưu_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (Program.opt == 1) //them
                {
                    ...
                    object[] values = { tb_tenloainhanvien.Text, Convert.ToString(tb_ghichu.Text) };
                ...
                if (Program.opt == 2) //sua
                {
                    ...
                    object[] values = { Int32.Parse(tb_id.Text), tb_tenloainhanvien.Text, Convert.ToString(tb_ghichu.Text) };
                    int a = connection.Excute_Sql(sql, CommandType.StoredProcedure, para, values);
                    if (a != 0)
                    {
        ...
        private void ChiTietLoaiNV_Load(object sender, EventArgs e)
        {
            ...
            if (Program.opt == 1) //them
            {
                ...
                tb_tenloainhanvien.Text = "";
                tb_ghichu.Text = "";
               
                tb_id.ReadOnly = true;

                tb_id.Text = Program.Quanlynhanviensql.TaoMaLoaiNhanVien().ToString();
            }
            if (Program.opt == 2) // sua
            {
                tb_id.ReadOnly = true;
